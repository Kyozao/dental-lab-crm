...
import {
  ...
  updateEmployeeLaborCostsForLoggedLab,
} from "../../employees.service";
import { parseUpdateEmployeeLaborCostsInput } from "../../employees.schemas";
...
export async function PUT(
  ...
  const parsed = parseUpdateEmployeeLaborCostsInput(payload.data);
  if (!parsed.success) {
  ...
  try {
    await updateEmployeeLaborCostsForLoggedLab(user_id, id, parsed.data);
    ...
    console.error("[PUT /api/employees/:id/labor-costs]", error);
    return NextResponse.json(
      { error: "Failed to update employee labor costs." },
      { status: 500 },
    );
  }
}
