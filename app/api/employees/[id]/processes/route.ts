...
import {
  ...
  updateEmployeeProcessesForLoggedLab,
} from "../../employees.service";
import { parseUpdateEmployeeProcessesInput } from "../../employees.schemas";
...
export async function PUT(
  ...
  const parsed = parseUpdateEmployeeProcessesInput(payload.data);
  if (!parsed.success) {
  ...
  try {
    const employee = await updateEmployeeProcessesForLoggedLab(
      user_id,
      id,
      parsed.data,
    );
    ...
    console.error("[PUT /api/employees/:id/processes]", error);
    return NextResponse.json(
      { error: "Failed to update employee processes." },
      { status: 500 },
    );
  }
}
