...
import {
  ...
  updateEmployeeProductivityForLoggedLab,
} from "../../employees.service";
import { parseUpdateEmployeeProductivityInput } from "../../employees.schemas";
...
export async function PUT(
  ...
  const parsed = parseUpdateEmployeeProductivityInput(payload.data);
  if (!parsed.success) {
  ...
  try {
    await updateEmployeeProductivityForLoggedLab(user_id, id, parsed.data);
    ...
    console.error("[PUT /api/employees/:id/productivity]", error);
    return NextResponse.json(
      { error: "Failed to update employee productivity." },
      { status: 500 },
    );
  }
}
